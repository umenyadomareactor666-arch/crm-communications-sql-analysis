/*
CRM Communications Effectiveness Analysis
Goal: evaluate CRM campaign effectiveness and identify which communications, channels,
and client segments generate the strongest response.

Key SQL skills shown:
- data profiling
- CTE
- JOIN
- CASE WHEN
...
-- 1. Source profiling: table sizes
...
-- 2. Data quality: duplicates in campaign audience
SELECT
    campaign_id,
    client_id,
...
-- 3. Data quality: missing key fields
SELECT
...
-- 5. Communication funnel by campaign
WITH event_flags AS (
    SELECT
        m.message_id,
        m.campaign_id,
...
-- 6. Test/control uplift by campaign
WITH conversions AS (
    SELECT
        ca.campaign_id,
        ca.client_id,
...
-- 8. Retargeting list: clients who clicked but did not apply in 7 days
...
-- 9. Validation: event timestamp cannot be earlier than sent_at
SELECT
    m.message_id,
    m.sent_at,
    e.event_type,
...
-- 10. Validation: control group clients should not receive messages in the same campaign
SELECT
    ca.campaign_id,
    ca.client_id,
    ca.group_type,
